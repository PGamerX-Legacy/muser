  ...
  client.channels.fetch("783581253001150494").then(async (channel) => {
    ...
    let embed =
        new Discord.MessageEmbed()
            ...
            .setDescription(`${user_info.username}#${
                user_info
                    .discriminator} just voted for [Muser](https://top.gg/bot/763418289689985035). You can vote too at [Top.GG](https://top.gg/bot/763418289689985035/vote)`)
            .setColor("RED")
...
const commandFiles =
    fs.readdirSync("./scommands").filter((file) => file.endsWith(".js"));
...
client.on("voiceStateUpdate", async (oldState, newState) => {
  ...
  if (oldState.channel.members.size > 1) {
    ...
    if (!timeouts.get(oldState.guild.id)) {
      ...
      timeouts.set(
          oldState.guild.id, setTimeout(async () => {
            const embed =
                new Discord.MessageEmbed()
...
client.once("ready", () => {
  ...
  client.user.setPresence({
    activities : [
      {
        name :
            `in ${client.guilds.cache.size} servers | Rip Groovy and Rythm o7`,
      },
    ],
    status : "online",
  });
  setInterval(() => {
    client.user.setPresence({
      activities : [
        {
          name : `in ${
              client.guilds.cache.size} servers | Rip Groovy and Rythm o7`,
        },
      ],
      status : "online",
    });
...
Volume : ${queue.volume}`)
                  .addField(`Requested by`,
                            `${song.member.user.username}#${
                                song.member.user.discriminator}`)
                  .setColor("RED")
                  .setThumbnail(song.thumbnail);
          queue.textChannel.send({embeds : [ embed ]});
        })
...
Volume : ${queue.volume}`)
                  .addField(`Requested by`,
                            `${playlist.member.user.username}#${
                                playlist.member.user.discriminator}`)
                  .setColor("RED")
    ...
    // DisTubeOptions.searchSongs = true
    .on("searchCancel",
        (message) => queue.textChannel.send(`Searching canceled`))
    .on("error",
        (channel, error) => channel.send(` An error encountered: ${error}`))
    .on("empty", (queue) => {
      const embed =
          new Discord.MessageEmbed()
              .setTitle(`Channel empty`)
